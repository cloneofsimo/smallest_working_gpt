...
class attention_layer(nn.Module):
    def __init__(self):
        super().__init__()
        self.k_proj = nn.Linear(emb_s, emb_s)
        self.q_proj = nn.Linear(emb_s, emb_s)
        self.v_proj = nn.Linear(emb_s, emb_s)
    ...
    def forward_multi_attn(self, x):
        ...
        x = self.dp(self.proj(multihead_attn))
        return x
...
class GPT(nn.Module):
    ...
    def forward(self, idx, targets=None, step_size = None):
        _, t = idx.size()
